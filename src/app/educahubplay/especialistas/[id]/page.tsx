...
export default function EspecialistaDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      if (useMock) {
        // No mock data for educahubplay specialists yet - show placeholder
        setService({
          id,
          nome: "Especialista Exemplo",
          descricao: "Este e um especialista de exemplo. Dados reais serao carregados quando o Firebase estiver configurado.",
          categoria: "Pedagogia",
          tags: ["Gamificacao", "E-learning"],
  ...
  if (!service) {
    return (
      <div className="text-center py-20 space-y-4">
        <p className="text-muted">Especialista nao encontrado.</p>
        <Link href="/educahubplay/especialistas" className="text-violet-400 hover:underline text-sm">
      ...
      <div className="bg-card border border-border rounded-2xl p-8 space-y-6">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-4">
            ...
            <Badge color="#8b5cf6">{service.categoria}</Badge>
          </div>
        ...
        {service.tags && service.tags.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {service.tags.map((tag) => (
              <Badge key={tag} className="bg-violet-500/10 text-violet-400">
                {tag}
              </Badge>
            ))}
          </div>
        ...
        <div className="space-y-3">
          ...
          {service.contato?.email && (
            <div className="flex items-center gap-2 text-sm text-muted">
              <Mail size={16} />
              <a href={`mailto:${service.contato.email}`} className="hover:text-violet-400">
                {service.contato.email}
              </a>
            </div>
          )}
          ...
          {service.contato?.site && (
            <div className="flex items-center gap-2 text-sm text-muted">
              <Globe size={16} />
              <a href={service.contato.site} target="_blank" rel="noopener noreferrer" className="hover:text-violet-400">
                {service.contato.site}
              </a>
            </div>
          )}
